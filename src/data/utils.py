...
#         y['price'] = y_scaler.fit_transform(y_array)
    ...
    X['bedrooms'] = pd.Categorical(
        X['bedrooms']) if categorize_bedrooms else X['bedrooms']
...
def map_model(X, model, scaler=None):
    ...
    lats, lat_step = np.linspace(
        X['latitude'].min(), X['latitude'].max(), num=300, retstep=True)
    longs, long_step = np.linspace(
        X['longitude'].min(), X['longitude'].max(), num=600, retstep=True)
    ...
    if scaler:
        df[['date', 'latitude', 'longitude', 'area']] = scaler.inverse_transform(
            df[['date', 'latitude', 'longitude', 'area']])

    bedrooms = df['bedrooms'].mode()[0] if 'bedrooms' in cols else 2
    unit_type = df['unit_type'].mode(
    )[0] if 'unit_type' in cols else 'apartment'
    title = "Price for a {}sqft, {} bed {}".format(
        df['area'].median(), bedrooms, unit_type)
...
def time_evolution(X, model):
    ...
    for d in dates:
        ...
        df['unit_type'] = pd.Categorical(df['unit_type'])
        df = df[['date', 'latitude', 'longitude', 'area',
                 'bedrooms', 'pets', 'furnished', 'unit_type']]
